...
def display_summary(series, df):
    ...
    table += '</table>'
    table += '<p><b><a href="https://github.com/wragge/ozglam-workbench-naa-wap/blob/master/RecordSearch/data/{}.csv">Download the complete CSV file</a></b></p>'.format(series.replace('/', '-'))
...
def make_summary_all(df):
    ...
    summary['date_from'] = df['date_from'].min()
    summary['date_to'] = df['date_to'].max()
    access_status = {}
